...
#import "HBXVideoListViewController.h"
...
@interface HBXVideoListViewController ()
<UITableViewDelegate,
UITableViewDataSource,
ZJLiveFileCacheManagerDelegate>
...
@implementation HBXVideoListViewController
...
- (void)loadVideoList {
    NSArray *list = [HBXVideoModel getVideoList];
...
- (void)startDownLoad:(UIButton *)sender {
    if ([self.textFiled.text containsString:@"http"] && self.textFiled.text.length > 0) {
        ...
        [HBXVideoModel addNewDownLoadWithUrl:url];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ShowVideoViewController *vc = [[ShowVideoViewController alloc] initWithUrl:dict[@"url"]];
