...
- (void)startAnimationWithView:(UIView *)animationView durtion:(CGFloat)time {
    animationView.transform =  CGAffineTransformMakeScale(0.05,0.05);
...
- (void)startAnimationWithView:(UIView *)animationView durtion:(CGFloat)time complete:(void(^)(BOOL finish))complete {
    animationView.transform =  CGAffineTransformMakeScale(0.05,0.05);
    [UIView animateWithDuration:0.5 animations:^{
        animationView.transform =  CGAffineTransformIdentity;
    } completion:^(BOOL finished) {
...
//    DMLog(@"syp====setTHChidden==%d,class==%@",ishidden,[self className]);
//}
